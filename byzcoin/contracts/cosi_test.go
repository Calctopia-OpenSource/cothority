...
func TestCosi_Spawn(t *testing.T) {
	...
	rootTransactionBuf, err := protobuf.Encode(&rootTransaction)
	require.Nil(t, err)
	...
	require.Equal(t, result.RootTransaction, rootTransaction)
...
func TestCosi_Scenario(t *testing.T) {
	...
	genesisMsg, err := byzcoin.DefaultGenesisMsg(byzcoin.CurrentVersion, roster,
		[]string{"spawn:value", "spawn:cosi", "invoke:cosi.addProof"}, signer.Identity())
	require.Nil(t, err)
	gDarc := &genesisMsg.GenesisDarc
	...
	require.Equal(t, result.RootTransaction, rootTransaction)
	require.Equal(t, len(result.RootTransaction.Instructions), 1)
	// require.Equal(t, result.RootTransaction.Instructions[0].Spawn.Args.Search("value"), 1)
	require.Equal(t, result.ExpireSec, expireSecInt)
	require.NotEmpty(t, result.Timestamp)
	require.Empty(t, result.RootTransaction.Instructions[0].SignerIdentities)
	require.Empty(t, result.RootTransaction.Instructions[0].Signatures)
	...
	rootTransaction.Instructions[0].SignerIdentities = append(rootTransaction.Instructions[0].SignerIdentities, identity)
	rootTransaction.Instructions[0].Signatures = append(rootTransaction.Instructions[0].Signatures, signature)
	result.RootTransaction = rootTransaction
	...
	require.Equal(t, result.ExpireSec, expireSecInt)
	require.NotEmpty(t, result.Timestamp)
	require.NotEmpty(t, result.RootTransaction.Instructions[0].SignerIdentities)
	require.Equal(t, len(result.RootTransaction.Instructions[0].SignerIdentities), 1)
	// Surprisingly this test won't work. But by using Equal() will.
	// require.Equal(t, result.RootTransaction.Instructions[0].SignerIdentities[0], identity)
	require.True(t, identity.Equal(&result.RootTransaction.Instructions[0].SignerIdentities[0]))

	require.NotEmpty(t, result.RootTransaction.Instructions[0].Signatures)
	require.Equal(t, len(result.RootTransaction.Instructions[0].Signatures), 1)
	require.Equal(t, result.RootTransaction.Instructions[0].Signatures[0], signature)
	...
	require.NotEmpty(t, result.Timestamp)
	require.NotEmpty(t, result.RootTransaction.Instructions[0].SignerIdentities)
	require.Equal(t, len(result.RootTransaction.Instructions[0].SignerIdentities), 2)
	// Surprisingly this test won't work. But by using Equal() will.
	// require.Equal(t, result.RootTransaction.Instructions[0].SignerIdentities[0], identity)
	require.True(t, identity.Equal(&result.RootTransaction.Instructions[0].SignerIdentities[1]))

	require.NotEmpty(t, result.RootTransaction.Instructions[0].Signatures)
	require.Equal(t, len(result.RootTransaction.Instructions[0].Signatures), 2)
	require.Equal(t, result.RootTransaction.Instructions[0].Signatures[1], signature)
